...
function renderShippingThreeChoice() {
  ...
  const showThree = state.fulfillment === "shipping" && allowedMixCount() === 3;
  const total = totalCookies();
  const remainder = total % 3;

  // If the full order divides evenly by 3, no special choice is needed.
  const showChoice = showThree && remainder !== 0;
  wrap.hidden = !showChoice;
  if (!showChoice) { wrap.innerHTML = ""; return; }

  if (!state.selectedFlavors.includes(state.shippingSixFlavor)) {
    state.shippingSixFlavor = state.selectedFlavors[2];
  }
  ...
  if (remainder === 2) {
    // Example: 20 -> 7 + 7 + 6, 80 -> 27 + 27 + 26.
    // One flavor is one cookie short; customer chooses which one.
    prompt = "One flavor will have one fewer cookie. Choose which flavor you’d like to have one less.";
    detail = `${total} cookies split as evenly as possible: ${base + 1} + ${base + 1} + ${base}.`;
    chosenQty = base;
  } else {
    // Example: 40 -> 14 + 13 + 13. With this remainder, keeping the
    // requested total means one flavor receives the extra cookie.
    prompt = "One flavor will have one extra cookie. Choose which flavor you’d like to have one more.";
    detail = `${total} cookies split as evenly as possible: ${base + 1} + ${base} + ${base}.`;
    chosenQty = base + 1;
  }
  ...
  state.selectedFlavors.slice(0,3).forEach(key => {
    ...
    btn.textContent = `${flavorNames[key]} — ${chosenQty}`;
