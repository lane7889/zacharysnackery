...
function validMixOptions() {
  ...
  if (state.fulfillment === "shipping") {
    // Shipping boxes contain 20 cookies, so two flavors split evenly (10/10 per box).
    // Three flavors use 4-cookie units (8/8/4 for one box).
    return { 1: true, 2: total >= 20, 3: total >= 20 && total % 4 === 0 };
  }
...
function render() {
  ...
  // The local-radius checkbox is only relevant to local delivery; remove it for shipping.
  $("local-radius-confirm").hidden = state.fulfillment !== "local";
